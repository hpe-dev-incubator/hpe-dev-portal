...
exports.createPages = async ({ actions }) => {
    ...
    getSpecialBadges.data.forEach(({ id, title, description, badgeImg }) => {
      createPage({
        ...
        component: require.resolve(
          './src/pages/Home/workshops/template.js',
        ),
        context: {
          specialBadgeId: id,
          title,
      ...
      console.log(
        `Create pages /Home/workshops/${id - 1}/special-badge from ${id}`,
      );
      console.log('------------------------------');
    });
  } catch (error) {
  ...
  //     console.log('------------------------------');
      ...
      createPage({
        path: `/Home/workshop/${id}`,
        component: require.resolve('./src/pages/Home/replays/template.js'),
        context: {
          workshopId: id,
          workshopTitle: name,
