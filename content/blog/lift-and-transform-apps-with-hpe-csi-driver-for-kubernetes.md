...
Do you run your applications in virtual machines on VMware today using the same patterns once employed by bare-metal paradigms? Just a bit more efficient from a resource consumption perspective, but what about the lifecycle of the application, its dependencies - including libraries, host operating systems and various mnemonics to keep the application up and running?

Containerization and Kubernetes in particular help businesses to move away from imperative operational models which require multiple teams to perform tedious tasks on a regular basis towards a declarative model where there’s a clean separation of concerns with a high degree of automation and abstractions that makes sense for running a high-performance technology company. 
...
The TL;DR version of this blog post is available as an instructional lightboard video [published on YouTube](https://www.youtube.com/watch?v=M7t_qPe3i5E). The blog concretizes what’s being outlined in the lightboard video. 

[![YouTube](/img/untitled.jpg)](https://www.youtube.com/watch?v=M7t_qPe3i5E)

Still want the details? Let’s get started!

# Origin state

In the following examples we’re going to use a MariaDB database with the blessed [Employees database](https://github.com/datacharmer/test_db) as an example workload. It provides simple means to validate its contents to ensure the contents are intact throughout its journey.
...
In the remaining tests throughout this blog, only the summary results will be displayed.

# Destination state

When the transition has completed, the Employees database should be running on a Kubernetes cluster using persistent storage provided by the HPE CSI Driver for Kubernetes backed by a HPE Nimble Storage array. The procedures are similar regardless of backend, HPE Alletra and Primera included.

![MariaDB running on Kubernetes](/img/slide2.png "MariaDB running on Kubernetes")

While the destination state is a virtualized Kubernetes cluster within the same vSphere environment, that is not a requirement, it can be any hypervisor, bare-metal or otherwise as long as the destination cluster is compatible with the HPE CSI Driver for Kubernetes and any of the supported backends.
...
Once the disk(s) have been migrated over the HPE Nimble Storage array, each individual virtual disk is represented as a standalone volume along with the VMs metadata volumes.
...
In this next phase, all configuration and testing are performed directly from the Kubernetes cluster. An admin needs to install the HPE CSI Driver for Kubernetes, create a `Secret` that represents the backend user or tenant and setup a `StorageClass` designed to allow importing of volumes.
...
For the final production instance, we’re going to use the [Bitnami MariaDB Helm chart](https://artifacthub.io/packages/helm/bitnami/mariadb). For this chart to install cleanly and be able to bring up the database, the following considerations needs to be taken into account:
...
If things dont’ come up properly, which is very likely it won’t on the first couple of iterations, read logs, error messages and course correct accordingly until the database come up cleanly. At this point, in a production scenario, test clients could be attached to further validate and solidify that moving the application to Kubernetes will actually work.

# The endgame

Up until this step, there’s been zero downtime or disruption to the source application. And the last phase is a one-way street. Ensure clients can connect to the destination database instance before proceeding.
...
The only change we need to make is the `annotations` stanza. Changing the directive from `importVolAsClone` to `importVolumeName`. We also need to “force” the import as the volume we’re importing already have residual vSphere application metadata associated with it. That will be overwritten by the HPE CSI Driver when forcing the import.
...
It’s also safe to move the volume out of the vVol datastore folder, it’s a non-disruptive operation and may be carried out by a storage administrator.
...
I’m gearing up for [HPE Discover 2022](http://hpe.com/discover). If you’d like to connect and talk shop, I’ll be in the HPE Developer Community Hack Shack on the main expo floor. [We have treasure hunts, hacking challenges and meetups](https://developer.hpe.com/blog/don%E2%80%99t-miss-all-things-software-in-the-hack-shack-at-hpe-discover/). Find my session “Get Started with Persistent Storage for Kubernetes with the HPE CSI Driver” in the HPE Discover [content catalog](https://content.attend.hpe.com/go/agendabuilder.sessions/?l=1049&sid=24991_10161&locale=en_US).

![Get Started with Persistent Storage for Kubernetes with the HPE CSI Driver](/img/hpe-dlv-2022-csi-hackshack-revc.png "Get Started with Persistent Storage for Kubernetes with the HPE CSI Driver")

The team is also on the HPE Developer Community Slack, sign up and login to join the conversation on everything HPE Developer related. See you there!