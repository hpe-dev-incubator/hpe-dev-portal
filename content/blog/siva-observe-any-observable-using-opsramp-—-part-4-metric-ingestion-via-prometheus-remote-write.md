...
In Part 3, I tested the local stack end-to-end using `otel-cli` and `promtool`, confirming that every component of the pipeline works correctly in isolation. Now I am ready to write the actual Python instrumentation code and push real metrics into OpsRamp.

This article covers the complete metric ingestion path: from the Redfish emulator through the OTel Python SDK, through the OTel Collector, through Prometheus `remote_write`, and finally into OpsRamp's Metrics Explorer. I will walk through the code in detail — the per-chassis OTel Resource design, the metric instrument definitions, the poll cycle implementation — and then show exactly how to configure Prometheus `remote_write` and verify the configuration in OpsRamp.
...
Before diving into code, let me make the complete metric flow explicit.
...
The `resource_to_telemetry_conversion: enabled` setting in the Collector's Prometheus exporter is critical. Without it, OTel Resource attributes (like `type`, `uuid`, `host.name`) remain in the resource scope and are not promoted to Prometheus metric labels. With it enabled, every metric carries all Resource attributes as labels — which is exactly what OpsRamp needs to see.
...
The `uuid` value must be the `resourceUUID` that OpsRamp assigned when the resource was created via its REST API. This is not a locally generated UUID — it is the specific identifier OpsRamp uses internally for each managed resource.
...
Before any metrics are sent, Phase 1 creates all 34 resources in OpsRamp and populates the ResourceContext registry. I run this once on agent startup.

### REST API flow

The OpsRamp resource creation API requires `global:manage` scope credentials — different from the `logs:write, metrics:write` credentials used by the Collector.
...
def create_or_update_resource(payload: dict) -> str | None:
    """
    Create a resource via OpsRamp REST API.

    OpsRamp returns resourceUUID in the response body:
...
With per-chassis providers, each chassis gets its own `host.name="Chassis-1"` and `uuid="d3878adb-..."`. OpsRamp associates each metric with the correct chassis resource.
...
def make_chassis_resource(chassis_id: str, chassis_data: dict) -> Resource:
    ...
    IMPORTANT: Called after Phase 1 so ResourceContext has the real
    OpsRamp uuid. The uuid is attached to every metric the resulting
    MeterProvider produces.

    OpsRamp resource association
    ----------------------------
    OpsRamp reads these attributes from OTLP metric payloads and
...
With metrics flowing from the agent through the Collector into Prometheus, the final step is configuring Prometheus to push them to OpsRamp via `remote_write`.

### Setting up remote_write in OpsRamp

Before configuring `prometheus.yml`, I need to set up the remote_write integration in OpsRamp:
...
global:
  ...
  # These help OpsRamp identify the source of the metrics.
  external_labels:
...
# OpsRamp remote_write configuration
...
remote_write:
  - url: "https://<your-tenant>.opsramp.com/api/v2/metric-ingest/prometheus/remote-write"
    ...
    # Write relabel configs — filter and preserve labels for OpsRamp
...
## Verifying metric ingestion in OpsRamp

After the agent runs for at least one poll cycle and Prometheus pushes via remote_write, verify metrics appear in OpsRamp.
...
### Verify in OpsRamp Metrics Explorer
...
This is the payoff: OpsRamp is now able to correlate metrics with resources, display them in the resource context, and apply its AIOps capabilities on top.
...
**Metrics visible in Prometheus but not in OpsRamp**
...
I have now built the complete metric ingestion path — from Redfish hardware data through the OTel Python SDK, through the Collector, through Prometheus remote_write, and into OpsRamp's Metrics Explorer with full resource association.

The key insight from this article is that resource association requires precise coordination across three layers: Phase 1 provisioning creates the resources and captures their UUIDs, ResourceContext propagates those UUIDs to the OTel instrumentation layer, and `resource_to_telemetry_conversion` ensures they appear as Prometheus labels that OpsRamp can match.

In the next article I will cover log and event ingestion — a different path that goes directly from the OTel Collector to OpsRamp's log endpoint via OTLP/gRPC, carrying per-resource association through a different mechanism: per-host `LoggerProvider` instances that attach the correct `type` and `uuid` to every log record at the resource level rather than the attribute level.