...
At the time of this release  (March 2024), the API resource to discover the cloud service provider (CSP) account id is not yet available as part of this released HPE GreenLake API for virtualization. However, I can still obtain the information about my GreenLake account Id at the CSP’s registration on HPE GreenLake Private Cloud Business Enterprise’s cloud account menu using the legacy CSP accounts list API `GET https://{baseUrl}/api/v1/csp-accounts`. Assuming that your AWS account had been onboarded into either HPE GreenLake Backup Recovery or HPE Private Cloud for Business Enterprise, this `account Id` will be used as one of the key-pair object in the JSON request body for the virtualization API `POST /virtualization/v1beta1/csp-machine-instances` to deploy virtual machine into cloud service provider.
