...
However, the challenge of using the open source [tool](https://openapi-generator.tech/) that is used to generate this client library, as of this blog posting, can only facilitate the OpenAPI Standard version 3.0 spec file as the input. However, majority of the HPE GreenLake APIs were documented using the OpenAPI Standard version 3.1.
...
> **Note:** that this conversion will not be required once the open-source tool such openapi-generator-cli eventually support OpenAPI 3.1 Standard spec file in the future. If you are using OpenAPI generator tool such as listed [here](https://openapi.tools/#converters), you may not required to use the conversion as documented in this blog.
