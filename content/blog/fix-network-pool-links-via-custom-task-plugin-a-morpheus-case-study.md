...
In a nutshell, we need a way to link a NetworkPoolIp entry to a ComputeServer, where a network interface falls within the same Network/NetworkPool.

These links exist via the refType and refId fields of the NetworkPoolIp object, as shown below.

![RefType and RefId fields](/img/morpheus-fix-pool-1.png "RefType and RefId fields")

Upon provisioning, the refType field is set to the literal ‘ComputeServer’ value, and the refId field is assigned the Id of the ComputeServer object itself.

When a NetworkPool is migrated/changed, or added to the Network after Instance provisioning, NetworkPoolIp records are potentially synchronized from an external IPAM pool. Even if the host records accurately reflect the ComputeServer hosts on the network, the refId link is broken, causing orphan records when resources are deprovisioned.

To reproduce, we will provision a single VM instance into a simple lab, then add the IP pool to the related network, afterward.

## Normal behavior

First, consider the normal day-to-day use case. We associate our network with the IP pool as shown below:

![Network IP Pool Association](/img/morrpheus-fix-pool-2.png "Network IP Pool Association")

Should we provision a VM instance into this network, a host record entry will be created in the IP pool:

![Host record Creation](/img/morrpheus-fix-pool-3..png "Host record Creation")

![Host record Creation#2](/img/morrpheus-fix-pool-4..png "Host record Creation#2")

By querying the host record via the REST API, we can see the link back to the ComputeServer within the instance:

![Host Record query via REST API](/img/morrpheus-fix-pool-5..png "Host Record query via REST API")

## Provisioning without IP pool
...
![Workflow Execution](/img/morrpheus-fix-pool-23..png "Workflow Execution")

Under the Executions tab we can view the run output of the task, showing which compute servers were updated.

![Execution Tab](/img/morrpheus-fix-pool-24..png "Execution Tab")

Re-running the REST API call confirms that refType/refId link was established:

![Host Record query via REST API](/img/morrpheus-fix-pool-25..png "Host Record query via REST API")

In large environments it would be impractical to execute the workflow for each IP pool by hand in the UI. For these environments, execute the workflow via the REST API. Provide the ID of the workflow in the URL and the ID of the IP Pool to  the networkPool body parameter to execute the POST request:
